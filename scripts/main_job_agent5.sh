...
LOCAL_DIR=/storage/scratch1/9/hzhang3050/soccer_twos_results/agent5
...
AGENT4_CHECKPOINT="${AGENT4_CHECKPOINT:-/storage/home/hcoda1/9/hzhang3050/scratch/soccer_twos_results/agent4/agent4_vs_baseline_ppo/Agent4Trainable_bd01c_00000_0_2026-04-19_22-36-31/checkpoint_000525}"
...
if [ ! -e "$AGENT4_CHECKPOINT" ]; then
    echo "Agent 4 checkpoint path not found: $AGENT4_CHECKPOINT"
    exit 1
fi
...
echo "=== SoccerTwos Agent 5 Job ==="
echo "Host: $(hostname)"
echo "Repo: $REPO"
echo "Python: $PYTHON_BIN"
echo "Results dir: $LOCAL_DIR"
echo "Agent4 checkpoint: $AGENT4_CHECKPOINT"
echo "Baseline checkpoint: $BASELINE_CHECKPOINT"
echo "Opponent mix: baseline=0.70 self_1=0.15 self_2=0.10 self_3=0.05"

"$PYTHON_BIN" -u train_agent5_vs_baseline.py \
    --timesteps "$TIMESTEPS" \
    --num-workers "$NUM_WORKERS" \
    --num-gpus "$NUM_GPUS" \
    --num-envs-per-worker "$NUM_ENVS_PER_WORKER" \
    --local-dir "$LOCAL_DIR" \
    --agent4-checkpoint "$AGENT4_CHECKPOINT" \
