...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Agent 5: fine-tune Agent 4 checkpoint to beat the CEIA baseline more reliably."
    )
    ...
    parser.add_argument("--experiment-name", default="agent5_vs_baseline_ppo")
    parser.add_argument(
        "--agent4-checkpoint",
        required=True,
        help="Path to the Agent 4 checkpoint file or containing directory.",
    )
    parser.add_argument(
        "--baseline-checkpoint",
        default=str(
    ...
    parser.add_argument("--eval-port-stride", type=int, default=20)
    ...
    parser.add_argument("--reward-anneal-frac", type=float, default=0.5)
    parser.add_argument("--baseline-prob", type=float, default=0.70)
    parser.add_argument("--self1-prob", type=float, default=0.15)
    parser.add_argument("--self2-prob", type=float, default=0.10)
    parser.add_argument("--self3-prob", type=float, default=0.05)
...
def load_available_policy_weights(checkpoint_path: Path):
    with checkpoint_path.open("rb") as f:
        checkpoint_data = pickle.load(f)
    worker_state = pickle.loads(checkpoint_data["worker"])
    states = {}
...
def is_port_conflict_error(exc: Exception) -> bool:
    current = exc
    while current is not None:
        message = str(current)
        if type(current).__name__ == "UnityWorkerInUseException":
            return True
        if "Address already in use" in message:
            return True
        if "worker number 0 is still in use" in message:
            return True
        current = current.__cause__ or current.__context__
    return False


def evaluate_vs_baseline_with_retry(
    trainer: PPOTrainer,
    episodes: int,
    base_port: int,
    port_stride: int,
    ...
    for attempt in range(max_attempts):
        port = base_port + attempt * port_stride
        try:
            ...
            evaluation["eval_base_port"] = port
            evaluation["eval_attempt"] = attempt + 1
            return evaluation
        except Exception as exc:
            last_exc = exc
            if not is_port_conflict_error(exc):
                raise
            print(
                f"Baseline evaluation port conflict on port {port}; "
                f"retrying with a new port (attempt {attempt + 1}/{max_attempts})."
            )
    raise RuntimeError(
        f"Baseline evaluation failed after {max_attempts} port attempts starting at {base_port}."
    ) from last_exc
...
class Agent5Trainable(Trainable):
    def setup(self, config):
        self.args = dict(config)
        self.variation = get_multiagent_player_variation()
        self.agent4_checkpoint = resolve_checkpoint_path(self.args["agent4_checkpoint"])
        ...
        self.env_name = "SoccerAgent5VsBaseline"
        ...
        with (Path(self.logdir) / "agent5_rllib_config.json").open("w", encoding="utf-8") as f:
        ...
        warm_states = load_available_policy_weights(self.agent4_checkpoint)
        baseline_weights = load_policy_weights(self.baseline_checkpoint, "default")
        train_weights = warm_states[TRAIN_POLICY_ID]
        self.trainer.set_weights(
            {
                TRAIN_POLICY_ID: train_weights,
                BASELINE_POLICY_ID: baseline_weights,
                "self_1": warm_states.get("self_1", train_weights),
                "self_2": warm_states.get("self_2", warm_states.get("self_1", train_weights)),
                "self_3": warm_states.get("self_3", warm_states.get("self_2", train_weights)),
            }
        )
    ...
    def step(self):
        ...
        if iteration % self.args["eval_interval"] == 0:
            eval_port = self.args["eval_base_port"] + iteration * self.args["eval_port_stride"]
            evaluation = evaluate_vs_baseline_with_retry(
                self.trainer,
                episodes=self.args["eval_episodes"],
                base_port=eval_port,
                port_stride=self.args["eval_port_stride"],
            )
    ...
    def save_checkpoint(self, checkpoint_dir):
        ...
        metadata_path = Path(str(checkpoint_path) + ".agent5_meta.pkl")
    ...
    def load_checkpoint(self, checkpoint):
        ...
        metadata_path = Path(str(checkpoint) + ".agent5_meta.pkl")
...
def main():
    args = parse_args()
    args.agent4_checkpoint = str(Path(args.agent4_checkpoint).expanduser().resolve())
    ...
    print("Done training Agent 5 vs baseline.")
    ray.shutdown()
